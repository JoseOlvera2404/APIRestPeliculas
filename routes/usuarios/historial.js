...
//Definición de la ruta /usuarios/historial
/**
 * @swagger
 * /usuarios/historial/{id_usuario}:
 *   get:
 *     summary: Obtiene la información del historial de un usuario en específico
 *     tags: [Usuarios]
 *     description: Devuelve la información del historial del usuario solicitado.
 *     parameters:
 *       - in: path
 *         name: id_usuario
 *         required: true 
    ...
    try {
        ...
        const result = await pool.request().
            ...
            query(`SELECT 
                    ...
                    FROM 
                        historial_reproduccion hr
                    JOIN 
                        pelicula p ON hr.id_pelicula = p.id_pelicula
                    JOIN 
                        director d ON p.id_director = d.id_director
                    WHERE 
                        hr.id_usuario = @id_usuario;
                    `);
